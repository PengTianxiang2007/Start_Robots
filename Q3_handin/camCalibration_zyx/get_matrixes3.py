...
def get_single_cb_matrixes(image, camera_matrix, distCoeffs, criteria,zeroZone=(-1, -1), winSize=(11, 11), cols=11, rows=8):
    ...
    cv2.drawChessboardCorners(image, (cols, rows), corners_fixed, flag)
...
def get_extrinsics_hand_matrix():
    ...
    """得到 camera->board矩阵"""
    images_path = current_dir / "data_20251224_handineye" / "images2"
...
def get_extrinsics_head_matrix():
    ...
    """得到 base->gripper 矩阵"""
    current_dir = Path(__file__).parent
    file_path = current_dir / "data_20251229_handtoeye" / "captures1.json"
    ...
    """得到 camera->board矩阵"""
    images_path = current_dir / "data_20251229_handtoeye" / "images2"
    criteria = (cv2.TERM_CRITERIA_EPS + cv2.TERM_CRITERIA_MAX_ITER, 30, 0.001)
    get_cb_matrixes(images_path, rot_cb, trs_cb, camera_matrix, distCoeffs, criteria)
    rot_bt, trs_bt, rot_cb, trs_cb = clean_missing_data(rot_bt, trs_bt, rot_cb, trs_cb)
