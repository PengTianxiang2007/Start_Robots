...
import visualizer
from get_matrixes2 import get_extrinsics_hand_matrix, get_extrinsics_head_matrix
...
class RGBDToPointCloudDeploy:
    ...
    - max_distance: 距离阈值（单位米），超过此距离的点会被过滤；为 None 或 <=0 则不启用。
    - z_min/z_max: Z 轴过滤范围（单位米），用于去除过近或过远的点；为 None 则不启用对应边界。
    """
    def __init__(
        self,
...
if __name__ == "__main__":
    ...
    parser.add_argument('--h5', type=str, help='HDF5 文件路径',  default="./box_4_re/episode0_frame1300.hdf5")
    ...
    extrinsics_hand = None
    extrinsics_hand = np.array([[-0.00970022,  0.00352649,  0.99994673, -0.09255557],
 [-0.99904838,  0.04249092, -0.00984135,  0.03886401],
 [-0.04252336, -0.99909063,  0.00311096,  0.08135386],
 [ 0.        ,  0.        ,  0.        ,  1.        ]]) 
    work_space = [[0, 0.7], [-0.5, 0.5],  [-0.4, 1]]

    rgbd2pcd_hand  = RGBDToPointCloudDeploy(
        num_points=args.num_points,
        intrinsics_json = './box_4_re/intrinsics/left_camera_intr_depth_d455.json',
        extrinsics= extrinsics_hand,
        work_space = work_space,
    )

    # 1b) 初始化 - 第三方相机
    extrinsics_head = np.array([[-0.01514023, -0.80190907,  0.59725423, -0.00341638],
 [-0.99974349,  0.02220348,  0.0044685 , -0.19394629],
 [-0.01684446, -0.59703337, -0.80203954,  0.64867338],
 [ 0.        ,  0.        ,  0.        ,  1.        ]]) 
    work_space = [[0, 0.7], [-0.5, 0.5],  [-0.4, 1]]
    ...
    rgbd2pcd_head = RGBDToPointCloudDeploy(
        num_points=args.num_points,
        intrinsics_json='./box_4_re/intrinsics/head_camera_intr_depth_d455.json',
        work_space = work_space,
